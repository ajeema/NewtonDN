...
parser = argparse.ArgumentParser(description='Newton Image Denoising')

parser.add_argument('--input_dir', default='dataset/test/input/', type=str, help='Input images')
...
model = NewtonDN_model().to(device)
# model()
...
for file_ in tqdm(files, desc="Processing images", ncols=100):
    ...
    img.close()
    with torch.no_grad():
        square_input_, mask, max_wh = overlapped_square(input_, kernel=model_img, stride=stride)
